...
function register_my_menu() {
    register_nav_menu('body-menu', __('Body Menu'));
}
...
register_nav_menus(array(
    ...
    'secondary' => __('Secondary Menu', 'panel'),
    'career' => __('Career Menu', 'panel'),
    'footer' => __('Footer Menu', 'panel')
));
...
//    }
    ...
    wp_enqueue_script('action', get_stylesheet_directory_uri() . '/js/action.js', array('jquery'), 1.1, true);
...
require_once(__DIR__ . '/include/ajaxFunctions.php');
...
function my_wp_mail_filter($args) {
    if (isset($_POST['action']) && $_POST['action'] == 'hugeit_validation_action' && isset($_POST['formId']) && $_POST['formId'] == '5') {
        if (strpos($args['message'], '<table') === false) {
            $args['message'] = ''
                    . '<div>Hello,</div> 

<div>Thanks for reaching out to Stradigi. We will review your email and respond as soon as possible.</div>  
 
<div>Looking forward to speaking, </div> 
 
<div>The Stradigi Team</div> '
                    . '';
        }
    }
    return $args;
